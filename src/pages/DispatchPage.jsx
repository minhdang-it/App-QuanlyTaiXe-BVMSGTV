...
export function DispatchPage() {
    ...
    async function cancelTrip(trip) {
        if (!window.confirm(`Hủy chuyến đi đến “${trip.destination}”?`))
            return;
        try {
    ...
    async function removeTrip(trip) {
        ...
        const vehicle = data.vehicles.find((item) => item.id === trip.vehicle_id);
        const confirmed = window.confirm(`XÓA VĨNH VIỄN chuyến ${vehicle?.plate_number ?? ''} đi “${trip.destination}” lúc ${formatDateTime(trip.scheduled_start)}?\n\nThao tác này không thể hoàn tác.`);
        if (!confirmed)
            return;
        try {
    ...
    async function fleetApproveTrip(trip) {
        try {
    ...
    async function directorApproveTrip(trip) {
        try {
            await updateTrip(trip.id, {
                status: 'assigned',
    ...
    async function rejectApproval(trip) {
      ...
      <div className="trip-filter-grid">
        ...
        <label>Loại chuyến
          <select value={purpose} onChange={(event) => setPurpose(event.target.value)}>
            ...
            {Object.keys(PURPOSE_LABELS).map((key) => <option key={key} value={key}>{PURPOSE_LABELS[key]}</option>)}
          </select>
        </label>
    ...
    {selectedTrip && <TripDetailModal trip={data.trips.find((item) => item.id === selectedTrip.id) ?? selectedTrip} canManage={canManage} onClose={() => setSelectedTrip(null)} onEdit={(trip) => { setSelectedTrip(null); setEditingTrip(trip); }} onCancel={(trip) => void cancelTrip(trip)} onDelete={(trip) => void removeTrip(trip)}/>}
  </>;
}
    ...
    {trip.status === 'active' && <section className="active-trip-live-card">
      ...
      {hasLiveLocation ? <>
        <div className="trip-detail-live-map">
          <iframe title={`Vị trí hiện tại xe ${vehicle?.plate_number ?? ''}`} src={`https://maps.google.com/maps?q=${liveLat},${liveLng}&z=16&output=embed`} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/>
...
function TripFormModal({ trip, initialRequestId, onClose, onSubmit }) {
    const { data } = useData();
    ...
    const [form, setForm] = useState({
        ...
        purpose: trip?.purpose ?? initialRequest?.purpose ?? 'community_exam',
    ...
    async function submit(event) {
        ...
        try {
            ...
            if (!Number.isFinite(newStart) || !Number.isFinite(newEnd) || newEnd <= newStart)
                throw new Error('Thời gian dự kiến về phải sau giờ xuất phát.');
            const conflict = data.trips.some((item) => {
                if (item.id === trip?.id || item.status === 'cancelled' || item.status === 'completed')
                    return false;
                if (item.vehicle_id !== form.vehicle_id && item.driver_id !== form.driver_id)
                    return false;
            ...
            if (conflict)
                throw new Error('Xe hoặc tài xế đã có chuyến trùng khoảng thời gian này.');
            await onSubmit({
    ...
    function applyApprovedRequest(requestId) {
        const request = approvedRequests.find((item) => item.id === requestId);
        setPlanFile(null);
        if (!request) {
            setForm({ ...form, vehicle_request_id: '', existing_plan_path: '', approved_plan: false });
            return;
        }
        setForm({
            ...form,
    ...
    <label>Loại chuyến<select value={form.purpose} onChange={(event) => setForm({ ...form, purpose: event.target.value })}>{Object.keys(PURPOSE_LABELS).map((key) => <option key={key} value={key}>{PURPOSE_LABELS[key]}</option>)}</select></label>
